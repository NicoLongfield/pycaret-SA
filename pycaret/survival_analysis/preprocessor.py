...
class Preprocessor:
    ...
    def _prepare_column_types(
        ...
        # Ordinal features
        X = feature_df if feature_df is not None else getattr(self, '_X', None)
        if ordinal_features:
            check_features_exist(ordinal_features.keys(), X)
        ...
        # Numerical features
        if numeric_features:
            check_features_exist(numeric_features, X)
            self._fxs["Numeric"] = numeric_features
        else:
            self._fxs["Numeric"] = list(X.select_dtypes(include="number").columns)

        # Date features
        if date_features:
            check_features_exist(date_features, X)
            self._fxs["Date"] = date_features
        else:
            self._fxs["Date"] = list(X.select_dtypes(include="datetime").columns)

        # Text features
        if text_features:
            check_features_exist(text_features, X)
