...
from pycaret.internal.preprocess.transformers_surv import (
    DropImputer,
    EmbedTextFeatures,
...
class PreprocessorSurvival:
    ...
    def _prepare_dataset(self, X, y=None):
        ...
        if isinstance(y, (tuple, np.ndarray, pd.Series)):
            if not isinstance(y, pd.Series):
                print("y is not a pandas Series. Converting to pandas Series.")
                # Check that y is two-dimensional
                ndim = np.array(y).ndim
                if ndim != 2:
                    raise ValueError(f"y should be two-dimensional, got ndim={ndim}.")
                ...
                y = Surv.from_dataframe(event=y[0], time=y[1], data=X[[y[0], y[1]]])
            ...
            y = to_df(data=X, columns=y)
            X = X.drop(columns=temp_y)
        ...
        for col in y.columns:
            if y[col].isna().any():
                raise ValueError(
                    f"{y[col].isna().sum()} missing values found in the target column: "
                    f"{col}. To proceed, remove the respective rows from the data. "
                )
    ...
    def _iterative_imputation(
        ...
        def get_prepare_estimator_for_categoricals_type(estimator, estimators_dict):
            # See pycaret_local.internal.preprocess.iterative_imputer
            fit_params = {}
